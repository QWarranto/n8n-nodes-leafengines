...
export class LeafEngines implements INodeType {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (operation === 'countyLookup') {
					...
					responseData = await leafEnginesApiRequest.call(this, {
						method: 'POST',
						url: '/county-lookup',
						body: {
							countyName,
					...
					const parallel = this.getNodeParameter('parallel', i) as number;
					...
					// Process in batches (simplified - in real implementation would use proper batching)
					responseData = {
					...
					// Note: Actual parallel processing would be implemented here
